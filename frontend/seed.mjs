...
async function seed() {
  ...
  try {
    ...
    const { count: banksCount, error: banksCheckError } = await supabase
      .from('banks')
      .select('*', { count: 'exact', head: true });

    if (banksCheckError) {
      throw new Error(`Tables not found. Please run schema SQL first. Error: ${banksCheckError.message}`);
    }
    ...
    const { count: finalBanksCount } = await supabase.from('banks').select('*', { count: 'exact', head: true });
    const { count: finalCardsCount } = await supabase.from('cards').select('*', { count: 'exact', head: true });
    const { count: finalCampaignsCount } = await supabase.from('campaigns').select('*', { count: 'exact', head: true });

    console.log(`   Banks: ${finalBanksCount}`);
    console.log(`   Cards: ${finalCardsCount}`);
    console.log(`   Campaigns: ${finalCampaignsCount}`);
